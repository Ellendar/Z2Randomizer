    ...
    public static Town LoadVanillaTown(ROM rom, TownType townType, bool disableContainerRequirements)
    {
        ...
        switch(townType)
        {
            ...
            case TownType.DARUNIA:
                ...
                //Upstab guy is the only map in wizard-shortened vanilla where an internal map connects to another internal map
                //so it didn't get loaded by LoadTown, so load/connect it here.
                map = rom.LoadTownMap((int)VanillaTownMap.DARUNIA_TRAINER, true);
